...
import { Activity, LogOut, LayoutDashboard, Building2, Settings, ShieldAlert, Mic2, Users, BellRing, Network, Menu, X } from 'lucide-react';
...
export default function DashboardLayout() {
  ...
  // Закриваємо меню при зміні маршруту (навігації)
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-[#09090b] flex text-white font-sans overflow-hidden">
      ...
      {/* Затемнення фону на мобільному при відкритому меню */}
      {isMobileMenuOpen && (
        <div 
      ...
      {/* Бічне меню (Sidebar) - Адаптивне */}
      <aside className={`fixed inset-y-0 left-0 z-50 w-72 bg-[#121214] border-r border-white/5 flex flex-col transform transition-transform duration-300 ease-in-out md:relative md:translate-x-0 ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        
        {/* Шапка меню з кнопкою закриття для мобільних */}
        <div className="h-16 flex items-center justify-between px-6 border-b border-white/5 shrink-0">
          <Link to="/" className="flex items-center gap-3 hover:bg-white/[0.02] transition-colors group cursor-pointer">
          ...
          <button onClick={() => setIsMobileMenuOpen(false)} className="md:hidden text-gray-400 hover:text-white p-2 bg-white/5 rounded-lg">
            <X size={20} />
          </button>
        </div>
          ...
          {currentMenu.map((item) => {
            ...
            return (
              <Link 
                key={item.path} 
                to={item.path}
                className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${isActive ? 'bg-blue-500/10 text-blue-400 border border-blue-500/20' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}
              >
                {item.icon}
        ...
        <div className="pb-6 text-center px-4 shrink-0">
      ...
      <main className="flex-1 flex flex-col min-w-0 h-screen relative">
        {/* Topbar */}
        <header className="h-16 bg-[#121214]/80 backdrop-blur-md border-b border-white/5 flex items-center px-4 sm:px-8 justify-between relative z-10 shrink-0">
          <div className="flex items-center gap-3 sm:gap-4 overflow-hidden">
            {/* Гамбургер меню для мобільних */}
            <button onClick={() => setIsMobileMenuOpen(true)} className="md:hidden p-2.5 -ml-2 text-gray-400 hover:text-white hover:bg-white/5 rounded-xl transition-colors">
              <Menu size={24} />
            </button>
            <h2 className="text-lg sm:text-xl font-semibold text-gray-200 truncate">Панель управління</h2>
          </div>
          
          <div className="flex items-center gap-2 sm:gap-3 bg-white/5 px-3 py-1.5 rounded-full border border-white/5 shrink-0">
            <span className="relative flex h-2 w-2">
              <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
              <span className="relative inline-flex rounded-full h-2 w-2 bg-emerald-500"></span>
            </span>
            <span className="text-[10px] sm:text-xs font-medium text-gray-300 hidden sm:inline-block">Система активна</span>
            <span className="text-[10px] sm:text-xs font-medium text-gray-300 sm:hidden">Активно</span>
          </div>
        </header>

        {/* Контент сторінки (Зменшені відступи для мобільних) */}
        <div className="flex-1 overflow-x-hidden overflow-y-auto p-4 sm:p-8 relative z-0">
          <Outlet />
        </div>
      </main>
    </div>
